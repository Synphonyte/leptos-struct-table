...
and then configure it for SSR like the following.
...
Please see the
[serverfn_sqlx example](https://github.com/Synphonyte/leptos-struct-table/blob/master/examples/serverfn_sqlx/Cargo.toml)
...
See the
[paginated_rest_datasource example](https://github.com/Synphonyte/leptos-struct-table/blob/master/examples/paginated_rest_datasource/src/data_provider.rs)
and the
[serverfn_sqlx example](https://github.com/Synphonyte/leptos-struct-table/blob/master/examples/serverfn_sqlx/src/data_provider.rs)
...
- **`sortable`** - Specifies that the table should be sortable. This makes the header titles clickable to control
  sorting. See the
  [simple example](https://github.com/synphonyte/leptos-struct-table/blob/master/examples/simple/src/main.rs)
...
- **`impl_vec_data_provider`** - If given, then [`TableDataProvider`] is automatically implemented for `Vec<ThisStruct>`
  to allow for easy local data use. See the
  [simple example](https://github.com/synphonyte/leptos-struct-table/blob/master/examples/simple/src/main.rs) for
  more information.
...
- **`renderer`** - Specifies the name of the cell renderer component. Used to customize the rendering of cells.
  ...
  If the feature `chrono` is enabled
  then [`DefaultNaiveDateTableCellRenderer`], [`DefaultNaiveDateTimeTableCellRenderer`] and
  [`DefaultNaiveTimeTableCellRenderer`] are used for [`chrono::NaiveDate`], [`chrono::NaiveDateTime`]
  and [`chrono::NaiveTime`] respectively.
...
- **`string`** - Specifies a format string. Currently only used for `NaiveDate`, `NaiveDateTime` and `NaiveTime`.
...
They are specified by using the various `...renderer` attributes on the struct or fields or props of
...
For more detailed information please have a look at the
...
[editable example](https://github.com/Synphonyte/leptos-struct-table/tree/master/examples/editable/src/main.rs)
